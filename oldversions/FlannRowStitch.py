...
class Stitcher:
    ...
    def stitch(self, images, ratio=0.75, reprojThresh=4.0,
        ...
        while num - 1 > j:
            ...
            M = self.flannMatchKeypoints(kpsR, kpsL,
    ...
    def flannMatchKeypoints(self, kpsR, kpsL, featuresR, featuresL,
                       ratio, reprojThresh):
        ...
        if len(good) > MIN_MATCH_COUNT:
            ptsR = np.float32([kp1[m.queryIdx].pt for m in good]).reshape(-1, 1, 2)
            ptsL = np.float32([kp2[m.trainIdx].pt for m in good]).reshape(-1, 1, 2)
